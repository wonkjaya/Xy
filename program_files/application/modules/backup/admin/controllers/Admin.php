...
//PRODUCT	
	function add_product(){
		...
		$data['categories']=$this->madmin->prd_cat();
	...
	function edit_category($id=0){
		...
		$edit=$this->madmin->edit_category($id);
		$data=array(
			'title'=>$title,
			'view_loader'=>$loader_view,
			'actv_menu'=>$active_menu,
...
//PERSONAL
	...
	function edit_profile(){
		$this->madmin->save_profile();
	}
// STORE
	function settings(){
		$this->load->helper('form');
		$data['categories']=$this->madmin->store_cat();
...
// SETTINGS STORE 
	function save_settings_store($type){
		$this->madmin->save_settings_store($type);
	}
